...
# Tanzania
download("https://www.dropbox.com/s/1st684dldym8ckt/TZ_geos_2017.csv.zip?raw=1", "TZ_geos_2017.csv.zip", mode="wb")
unzip("TZ_geos_2017.csv.zip", overwrite=T)
TZ_geos <- read.table("TZ_geos_2017.csv", header=T, sep=",")
TZ_geos <- na.omit(TZ_geos)
...
# Nigeria
download("https://www.dropbox.com/s/5pyj0huyf4k8fdy/NG_geos_2017.csv.zip?raw=1", "NG_geos_2017.csv.zip", mode="wb")
unzip("NG_geos_2017.csv.zip", overwrite=T)
NG_geos <- read.table("NG_geos_2017.csv", header=T, sep=",")
NG_geos <- na.omit(NG_geos)
...
# Ghana
download("https://www.dropbox.com/s/usbie5h0pxpybvq/GH_geos_2017.csv.zip?raw=1", "GH_geos_2017.csv.zip", mode="wb")
unzip("GH_geos_2017.csv.zip", overwrite=T)
GH_geos <- read.table("GH_geos_2017.csv", header=T, sep=",")
GH_geos <- na.omit(GH_geos)
...
# <alluvial> diagrams -----------------------------------------------------
alluvial(TZ_geof[,1:3], freq=TZ_geof$Freq, border=NA, col=ifelse(geof$BP == "Y", "red", "gray"))
alluvial(ET_geof[,1:3], freq=ET_geof$Freq, border=NA, col=ifelse(geof$BP == "Y", "red", "gray"))
alluvial(NG_geof[,1:3], freq=NG_geof$Freq, border=NA, col=ifelse(geof$BP == "Y", "red", "gray"))
alluvial(GH_geof[,1:3], freq=GH_geof$Freq, border=NA, col=ifelse(geof$BP == "Y", "red", "gray"))
